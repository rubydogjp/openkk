...
const RIGHT_GROUPS: PLGroup[] = [
  ...
  {
    title: "所得金額",
    rows: [{ label: "控除前の所得金額", note: "(33+37-42)", index: 43, height: 29 }],
  },
];

function buildGroupHtml(group: PLGroup, amounts: Record<number, number | null>): string {
  const headerStyle = [
    `height:${HEADER_H}px`,
    `display:flex`,
...
function buildPlHtml(amounts: Record<number, number | null>): string {
  const sectionStyle = `width:${NO_W + LABEL_W + AMOUNT_W}px`;
  const left = LEFT_GROUPS.map((g) => buildGroupHtml(g, amounts)).join("\n");
  const center = CENTER_GROUPS.map((g) => buildGroupHtml(g, amounts)).join("\n");
  const right = RIGHT_GROUPS.map((g) => buildGroupHtml(g, amounts)).join("\n");
...
export function buildFinancialStatementsBody(args: FinancialStatementsArgs): string {
  const { amounts, bsRows } = args;

  return (
    wrapPage("損益計算書 (P/L)", buildPlHtml(amounts), 1) +
...
export function buildFinancialStatementsDocument(args: FinancialStatementsArgs): string {
