...
describe("SQLite v1 to v2 migration", () => {
  it("adds constraints and preserves valid records", async () => {
    ...
    expect(db.selectValue(`SELECT value FROM openkk_meta WHERE key='schema_version'`))
      .toBe(String(SCHEMA_VERSION));
    expect(db.selectValue(`SELECT local_id FROM entries WHERE id='entry-1'`))
      .toBe("source-1");
    expect(db.selectValue(`SELECT json_type(data, '$.opening') FROM fiscal_periods WHERE id='fp-1'`))
      .toBeNull();
    expect(db.selectValue(`SELECT json_extract(data, '$.phase') FROM fiscal_periods WHERE id='fp-1'`))
      .toBe("pre_closing");
    expect(db.selectValue(`SELECT json_extract(data, '$.archiveStatus') FROM fiscal_periods WHERE id='fp-1'`))
      .toBe("active");
    expect(db.selectValue(`SELECT COUNT(*) FROM pre_closings WHERE fiscal_period_id='fp-1'`))
      .toBe(1);
    expect(db.selectValue(`SELECT COUNT(*) FROM closings WHERE fiscal_period_id='fp-1'`))
      .toBe(0);
    expect(db.selectValue(`SELECT id FROM openings WHERE fiscal_period_id='fp-1'`))
      .toBe("opening-1");
    expect(db.selectValue(`SELECT account_id FROM opening_balance_lines WHERE opening_id='opening-1'`))
      .toBe("acct_cash");
    expect(db.selectValue(`SELECT id FROM opening_journals WHERE opening_id='opening-1'`))
      .toBe("journal-1");
    expect(db.selectValue(`SELECT id FROM opening_journal_lines WHERE opening_id='opening-1'`))
      .toBe("journal-line-1");
    expect(db.selectValue(`SELECT COUNT(*) FROM entry_lines WHERE entry_id='entry-1'`))
      .toBe(1);
    ...
    expect(db.selectValue(`SELECT COUNT(*) FROM opening_balance_lines`)).toBe(0);
    expect(db.selectValue(`SELECT COUNT(*) FROM opening_journals`)).toBe(0);
    expect(db.selectValue(`SELECT COUNT(*) FROM opening_journal_lines`)).toBe(0);
