...
| ステージ遷移 | `pre_opening` → `journalizing` → `post_closing` の 3 段階 |
| 仮締め管理 | 仮締め完了フラグ (`provisionalClosingCompleted`) で仕訳のロック制御 |
...
| 機能 | 説明 |
|---|---|
| 固定資産の登録 | 名称・取得価額・現在簿価・耐用年数・事業按分率を入力 |
